...
class classification:
    ...
    def create_train_data(self):
        ...
        for image in os.listdir(self.Train_Data_Directory):
            ...
            img_data = np.asarray(equalize_hist(img_data) * 255, dtype='uint8')
    ...
    X_train = np.array([i[0] for i in train_data]).reshape(-1, Image_Size, Image_Size, 1)
    y_train = [i[1] for i in train_data]

    X_test = np.array([i[0] for i in test_data]).reshape(-1, Image_Size, Image_Size, 1)
